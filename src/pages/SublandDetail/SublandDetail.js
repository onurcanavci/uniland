...
const SublandDetail = () => {
    ...
    const isAuthorized = (address) => {
        ...
        if (sublandInfo.authorized_address === '0x0000000000000000000000000000000000000000') {
            return false
        }
        const currentTimestamp = Math.floor(Date.now() / 1000)
        return address === sublandInfo.authorized_address && currentTimestamp <= sublandInfo.authorized_until
    }
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        try {
            customContract.sublands(history.state.mainLandId, history.state.sublandId).then((response) => {
    ...
    const handleAuthorize = async (event) => {
        ...
        const dateString = authorizeField.until_date + 'T' + authorizeField.until_time;
        ...
        await transactionHandle(tx, `Subland authorized to ${authorizeField.to} until ${timeConverter(untilTimestamp)}`)
    }
                        ...
                        <p>Url: {sublandInfo.url}</p>
                        {
                            isAuthorized(sublandInfo.authorized_address) && (
                                <div>
                                    ...
                                    <p>Authorized Until: {timeConverter(sublandInfo.authorized_until)}</p>
                                </div>
                            )
                        }
                    </div>
